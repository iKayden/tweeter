$(document).ready(function() {
  ...
  const createTweetElement = (tweet) => {
    ...
    const $tweetArticle = $(`<article class="tweet-list-item">
      ...
      <img class="tweet-list-avatar" src="${tweet.user.avatars
      }" alt="Profile picture"/>
  ...
  $alertDiv.on("click", () => {
    //Second one making it hide on click
    $alertDiv.hide();
  });
  ...
  $tweetForm.on("submit", (e) => {
    //Capturing submitting phase event
    ...
    if (!$textValue) {
      ...
      $alertDiv
        .text("Tweet exceeds maximum length of the characters")
        .addClass("error-popup") // adding CSS markup for alert in our popup div
        .show(); // showing us the popup alert if it was used and closed before
      $alertDiv.delay(4000).fadeOut();
      return;
    ...
    $.ajax({
      ...
      .then((tweet) => {
        ...
        $(".counter").val(140); // !(Func Req. fix)backs the counter to
      })
      .catch((err) => {
        console.log(`${err}`);
      });
